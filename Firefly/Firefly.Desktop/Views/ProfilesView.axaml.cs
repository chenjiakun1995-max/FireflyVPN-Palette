...
public partial class ProfilesView : ReactiveUserControl<ProfilesViewModel>
{
    ...
    private static readonly HashSet<string> _protectedColumnTags =
    [
        "Address", "Port", "Network", "StreamSecurity"
    ];
    private readonly Dictionary<string, bool> _normalColumnVisibility = new(StringComparer.Ordinal);

    public Control SubscriptionGroupsTutorialTarget => lstGroup;
    public Control NodesTutorialTarget => lstProfiles;
    public Control RealDelayTutorialTarget => btnFastRealPing;
    public Control MixedTestTutorialTarget => menuMixedTestServer;

    public Control? GetFirstNodeTutorialTarget() => lstProfiles.GetVisualDescendants()
        .OfType<DataGridRow>()
        .FirstOrDefault(row => row.Index == 0);

    public void SelectFirstNodeForTutorial()
    {
        if (ViewModel?.ProfileItems.Count is not > 0)
        {
            return;
        }
    ...
    public ProfilesView()
    {
        ...
        lstGroup.SelectionChanged += (_, _) => Dispatcher.UIThread.Post(ApplyManagedGroupPresentation);
        lstProfiles.ContextMenu!.Opening += (_, _) => ApplyManagedGroupPresentation();
        ...
        RestoreUI();
        RepairLegacyProtectedColumnLayout();
        RememberNormalColumnVisibility();
    }
    ...
    private void RememberNormalColumnVisibility()
    {
        foreach (var column in lstProfiles.Columns)
        {
            if (column.Tag is string tag)
            {
                _normalColumnVisibility[tag] = column.IsVisible;
            }
        }
    }

    /// <summary>
    /// Earlier builds persisted the temporary hidden state used for a Worker
    /// group. Restore the default presentation only for that exact four-column
    /// signature, leaving normal individual user column choices untouched.
    /// </summary>
    private void RepairLegacyProtectedColumnLayout()
    {
        if (_config.UiItem.FireflyProtectedColumnLayoutMigrated)
        {
            return;
        }

        _config.UiItem.FireflyProtectedColumnLayoutMigrated = true;
        var protectedColumns = _config.UiItem.MainColumnItem
            .Where(item => _protectedColumnTags.Contains(item.Name))
            .ToList();
        if (protectedColumns.Count != _protectedColumnTags.Count || protectedColumns.Any(item => item.Width >= 0))
        {
            return;
        }

        // Remove only the corrupted entries. The XAML defaults then restore the
        // normal width and visibility for a locally managed subscription.
        foreach (var column in protectedColumns)
        {
            _config.UiItem.MainColumnItem.Remove(column);
        }

        foreach (var column in lstProfiles.Columns)
        {
            if (column.Tag is string tag && _protectedColumnTags.Contains(tag))
            {
    ...
    private void ApplyManagedGroupPresentation()
    {
        var protectedGroup = ViewModel?.IsProtectedPresentationGroup == true;
        foreach (var column in lstProfiles.Columns)
        {
            if (column.Tag is not string tag || !_protectedColumnTags.Contains(tag))
            {
                continue;
            }
            column.IsVisible = !protectedGroup && _normalColumnVisibility.GetValueOrDefault(tag, true);
        }

        // Keep connection and diagnostics actions available. Management,
        // disclosure, and export actions are not meaningful for Worker groups
        // (and are already blocked by ServiceLib as a second line of defense).
        foreach (var item in new[]
                 {
                     menuEditServer, menuCopyServer, menuRemoveServer, menuRemoveDuplicateServer,
                     menuRemoveInvalidServerResult, menuMoveToGroup, menuMoveTo, menuShareServer,
                     menuExportConfig, menuGenGroupServer
                 })
        {
            item.IsVisible = !protectedGroup;
        }
        separatorAfterSelectAll.IsVisible = !protectedGroup;
        separatorBeforeGenGroup.IsVisible = !protectedGroup;
    }
    ...
    private void StorageUI()
    {
        try
        {
            ...
            foreach (var item2 in lstProfiles.Columns)
            {
                ...
                var isVisible = item2.IsVisible == true;
                if (ViewModel?.IsProtectedPresentationGroup == true
                    && item2.Tag is string tag
                    && _protectedColumnTags.Contains(tag))
                {
                    // Worker-group presentation is temporary and must never
                    // overwrite the user's normal column preference.
                    isVisible = _normalColumnVisibility.GetValueOrDefault(tag, true);
                }

                lvColumnItem.Add(new()
                {
                    Name = (string)item2.Tag,
                    Width = (int)(isVisible ? item2.ActualWidth : -1),
