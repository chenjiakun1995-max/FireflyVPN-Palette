...
public static class ConfigHandler
{
    ...
    public static Config? LoadConfig()
    {
        ...
        config.GuiItem ??= new();
        if (!config.GuiItem.StatisticsPreferenceInitialized)
        {
            // Migrate prior upstream defaults once. Users can still turn the
            // setting off afterwards without it being re-enabled on restart.
            config.GuiItem.EnableStatistics = true;
            config.GuiItem.StatisticsPreferenceInitialized = true;
        }
        ...
        config.UiItem.StartupSplashDurationSeconds = config.UiItem.StartupSplashDurationSeconds <= 0
            ? 10
            : config.UiItem.StartupSplashDurationSeconds;
        ...
        config.ConstItem.FireflyKnownSubscriptionIds ??= [];
        config.ConstItem.FireflyUnreadSubscriptionIds ??= [];
    ...
    public static async Task<int> SetDefaultServer(Config config, List<ProfileItemModel> lstProfile)
    {
        ...
        {
            // Firefly-managed rows deliberately keep their address and port out
            // of the normal ProfileItem table. Resolve the visible rows through
            // the encrypted secret store before deciding that the group has no
            // usable node.
            var hydratedProfiles = await AppManager.Instance.ProfileItems(string.Empty) ?? [];
            var selectedIndex = SelectDefaultServerIndex(lstProfile, hydratedProfiles);
            if (selectedIndex.IsNotEmpty())
            {
                return await SetDefaultServerIndex(config, selectedIndex);
            }
        }

        var item = (await AppManager.Instance.ProfileItems(string.Empty))?.FirstOrDefault(t => t.Port > 0);
        return await SetDefaultServerIndex(config, item?.IndexId);
    }

    internal static string? SelectDefaultServerIndex(
        IReadOnlyCollection<ProfileItemModel> visibleProfiles,
        IReadOnlyCollection<ProfileItem> hydratedProfiles)
    {
        var directlyUsable = visibleProfiles.FirstOrDefault(t => t.Port > 0);
        if (directlyUsable is not null)
        {
            return directlyUsable.IndexId;
        }

        var visibleIds = visibleProfiles
            .Select(t => t.IndexId)
            .Where(t => t.IsNotEmpty())
            .ToHashSet(StringComparer.Ordinal);
        return hydratedProfiles.FirstOrDefault(t => visibleIds.Contains(t.IndexId)
                                                     && (t.Port > 0
                                                         || t.ConfigType is EConfigType.Custom or EConfigType.Outbound))
            ?.IndexId;
    }
    ...
    {
        return await EnsureDefaultServer(config);
    }

    /// <summary>
    /// Returns the current runtime-valid server or repairs the selection from
    /// hydrated profiles. Preferred subscription IDs are considered first so
    /// a freshly provisioned Worker group becomes usable immediately.
    /// </summary>
    public static async Task<ProfileItem?> EnsureDefaultServer(
        Config config,
        IEnumerable<string>? preferredSubscriptionIds = null)
    {
        ...
        foreach (var subscriptionId in preferredSubscriptionIds ?? [])
        {
            if (subscriptionId.IsNullOrEmpty())
            {
                continue;
            }
            selected = (await AppManager.Instance.ProfileItems(subscriptionId))?
        ...
        selected ??= (await AppManager.Instance.ProfileItems(string.Empty))?
            .FirstOrDefault(item => item.IsValid());
        if (selected is null)
        {
            return null;
        ...
        if (selected.Subid.IsNotEmpty())
        {
            config.SubIndexId = selected.Subid;
        }
        await SetDefaultServerIndex(config, selected.IndexId);
        return selected;
    }
    ...
    public static async Task<RetResult> AddGroupAllServer(Config config, SubItem? subItem)
    {
        ...
        if (FireflyManagedSubscriptionPolicy.IsManagedSubscription(subItem))
        {
