...
public partial class ProfilesViewModel : MyReactiveObject
{
    ...
    /// <summary>
    /// The synthetic "All" group includes Worker-provisioned nodes, so it is
    /// presented with the same disclosure-safe UI as a managed group. Locally
    /// created subscription groups retain the normal desktop controls.
    /// </summary>
    public bool IsProtectedPresentationGroup => SelectedSub is null
        || SelectedSub.Id.IsNullOrEmpty()
        || FireflyManagedSubscriptionPolicy.IsManagedSubscription(SelectedSub);
    ...
    public ProfilesViewModel()
    {
        ...
        this.WhenAnyValue(x => x.SelectedSub)
                ...
                .Select(y => y!)
                .Subscribe(async selected => await SubSelectedChangedAsync(selected));
        this.WhenAnyValue(
        ...
        // MainWindowViewModel awaits this exact task before starting the Worker
        // sync. Without a shared task, the initial database read can finish
        // after the Worker refresh and overwrite the newly received groups.
    ...
    private async Task SubSelectedChangedAsync(SubItem selectedSub)
    {
        await MarkSubscriptionGroupReadAsync(selectedSub);

        // Refresh even when the persisted ID already equals the selected ID.
        // On startup the ListBox selection is restored after the node content
        // arrives, and equality must not suppress the first visible refresh.
    ...
    public async Task RefreshSubscriptions()
    {
        ...
        var unreadSourceIds = (_config.ConstItem.FireflyUnreadSubscriptionIds ?? [])
            .ToHashSet(StringComparer.Ordinal);
        foreach (var subItem in subItems)
        {
            var sourceId = FireflyManagedSubscriptionPolicy.GetSourceId(subItem);
            subItem.IsUnread = sourceId.IsNotEmpty() && unreadSourceIds.Contains(sourceId);
        }
    ...
    private async Task MarkSubscriptionGroupReadAsync(SubItem selectedSub)
    {
        if (!selectedSub.IsUnread)
        {
            return;
        }

        selectedSub.IsUnread = false;
        var sourceId = FireflyManagedSubscriptionPolicy.GetSourceId(selectedSub);
        if (sourceId.IsNotEmpty()
            && _config.ConstItem.FireflyUnreadSubscriptionIds?.Remove(sourceId) == true)
        {
    ...
    private async Task<List<ProfileItemModel>?> GetProfileItemsEx(string subid, string filter)
    {
        ...
        foreach (var profile in lstModel)
        {
    ...
    public async Task EditServerAsync()
    {
        ...
        if (await RejectManagedDisclosure(item))
        {
            return;
        }
    ...
    public async Task RemoveServerAsync()
    {
        ...
        if (await RejectManagedDisclosure(lstSelected))
        {
            return;
        }
    ...
    {
        if (FireflyManagedSubscriptionPolicy.IsManagedSubscription(SelectedSub))
        {
    ...
    public async Task ServerSpeedtest(ESpeedActionType actionType)
    {
        ...
        if (actionType is ESpeedActionType.Mixedtest or ESpeedActionType.FastRealping)
        {
            ...
            lstSelected = await AppManager.Instance.ProfileItems(_config.SubIndexId);
        }
        else
        {
    ...
    private async Task EditSubAsync(bool blNew)
    {
        ...
        if (await AppManager.Instance.WindowDialog.ShowDialogAsync(subEditViewModel) == true)
        {
            ...
            {
                await SubSelectedChangedAsync(SelectedSub);
            }
        }
    }
    ...
    private static async Task<bool> RejectManagedDisclosure(ProfileItem? item)
    {
        if (!await FireflyManagedSubscriptionPolicy.ContainsManagedProfileAsync(item))
        {
            return false;
        }
    ...
    private static async Task<bool> RejectManagedDisclosure(IEnumerable<ProfileItem>? items)
    {
        if (!await FireflyManagedSubscriptionPolicy.ContainsManagedProfilesAsync(items))
        {
            return false;
        }
