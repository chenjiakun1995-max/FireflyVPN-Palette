...
public class Utils
{
    ...
    public static bool HasWritePermission()
    {
        try
        {
            ...
            if (IsFireflyInstallerDirectory(basePath))
            {
                return false;
            }
    ...
    private static bool IsFireflyInstallerDirectory(string basePath)
    {
        if (!IsWindows())
        {
            return false;
        ...
        var installLocation = WindowsUtils.RegReadValue(_fireflyInstallerRegistryPath, _fireflyInstallerRegistryValue, "");
        return installLocation.IsNotEmpty()
               && string.Equals(
                   Path.TrimEndingDirectorySeparator(basePath),
                   Path.TrimEndingDirectorySeparator(installLocation),
    ...
    private static void MigrateLegacyLocalAppData(string localAppData, string fireflyPath)
    {
        if (Interlocked.Exchange(ref _localAppDataMigrationAttempted, 1) != 0)
        {
            return;
        }

        var legacyPath = Path.Combine(localAppData, _legacyLocalAppDataDirectory);
        var legacyConfigPath = Path.Combine(legacyPath, "guiConfigs", Global.ConfigFileName);
        var fireflyConfigPath = Path.Combine(fireflyPath, "guiConfigs", Global.ConfigFileName);
        if (!File.Exists(legacyConfigPath) || File.Exists(fireflyConfigPath))
        {
            return;
        }
        ...
        try
        {
            // Firefly releases before this change used the upstream folder
            // name. Migrate the old configuration even if an empty new folder
            // has already been created, so upgrades keep the user's settings.
            if (!Directory.Exists(fireflyPath))
            {
                Directory.Move(legacyPath, fireflyPath);
                return;
            }

            Directory.CreateDirectory(fireflyPath);
            foreach (var sourcePath in Directory.EnumerateFileSystemEntries(legacyPath))
            {
                var destinationPath = Path.Combine(fireflyPath, Path.GetFileName(sourcePath));
        ...
        {
            // If another process owns a file, leave the legacy folder intact.
            // The application remains usable and a later run can be migrated
            // manually without risking data loss.
        }
    }
    ...
    {
        // Generated core files carry the complete active node configuration.
        // Keep them in the OS's per-user application-data location instead of
        // alongside the portable executable on supported desktop platforms.
        var tempPath = Utils.IsWindows() || Utils.IsMacOS()
            ? FireflyRuntimeArtifactPolicy.GetRuntimeConfigDirectory()
            : Path.Combine(StartupPath(), "binConfigs");
        if (!Directory.Exists(tempPath))
        {
            Directory.CreateDirectory(tempPath);
        }
